...
for id, creature in pairs(creatures) do
    ...
    if get_state(id)==CREATURE_EAT and get_type(id)==0 then    
        ...
        lcount=lcount+EATWEIGHT
    else 
        ng[1],ng[2]=ng[1]+x,ng[2]+y
        lcount=lcount+1
    end
    if get_state(id)==CREATURE_ATTACK then    
        ng[1],ng[2]=ng[1]+x*ENEMYWEIGHTAT,ng[2]+y*ENEMYWEIGHTAT
        lcount=lcount+ENEMYWEIGHTAT
    end
...
COUNT=realcount
WCOUNT=warriorcount
INNERCIRCLE=1*256+(1.1011^WCOUNT)*256
...
--g[1]=math.floor((math.cos((game_time()/100000)))*64*256+43*256)
--g[2]=math.floor((math.sin((game_time()/100000)))*42*256+32*256)

--print("x  :" .. (math.cos((game_time()/1000)/100)).." game time "..game_time())
--print("y  :" .. (math.sin((game_time()/1000)/100)).." game time "..game_time())
...
function Creature:move()
    ...
    elseif self:dist(x,y,self.wt[1],self.wt[2]) < 5 then
        ...
        if newc==nil then 
		newc=self:getrandomstep() 
	elseif self:dist(newc[1],newc[2],x,y) > 3*256 then
		newc=self:getrandomstep() 	
	end 
